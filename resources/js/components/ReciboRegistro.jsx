...
const ReciboRegistro = () => {
        ...
        axios
            ...
            .then(({ data }) => {
                ...
                const sc = dets.map((d) => ({
                    ...
                    nofactura:
                        d?.cuenta?.nofactura ??
                        d?.cuenta?.factura?.nofactura ??
                        d?.cuenta?.adm_factura?.nofactura ??
                        d?.cuenta?.adm_facturas?.nofactura ??
                        d?.cuenta?.adm_facturas?.no_factura ??
                        "",
                    ...
                    saldo_pendiente:
                        Number(d?.cuenta?.saldo_pendiente ?? 0) +
                        Number(d?.monto ?? 0),
    ...
    useEffect(() => {
        if (selectedCuentas.length > 0) {
            const total = selectedCuentas.reduce(
                (a, c) => a + (Number(c.monto_a_pagar) || 0),
                0
            );
    ...
    const handleSubmit = async (e) => {
        ...
        // Validaciones base
        if (!form.serie?.trim())
            return alertify.error("La serie es obligatoria.");
        if (
            !form.numero ||
        ...
        if (selectedCuentas.length > 0) {
            ...
            if (
                parseFloat(form.monto_recibido) >
                parseFloat(form.saldo_pendiente)
            )
                return alertify.error(
        ...
        const totalDetalle = detalle.reduce(
            (a, d) => a + (Number(d.monto) || 0),
            0
        );
        if (totalDetalle <= 0)
            return alertify.error("El total del recibo debe ser mayor a 0.");
    ...
    const columnas = useMemo(
        () => [
            ...
            // { accessorKey: "nofactura", header: "No. Interno", size: 120 },
            {
                ...
                accessorFn: (row) =>
                    row.nofactura ??
                    row?.factura?.nofactura ??
                    row?.adm_factura?.nofactura ??
            ...
            },
            {
                ...
                accessorFn: (row) =>
                    Number.parseFloat(row?.monto_original ?? 0),
            ...
            },
            {
                ...
                Cell: ({ row }) =>
                    fmt2.format(Number(row.original.monto_a_pagar ?? 0)),
                muiTableBodyCellProps: { align: "right" },
                size: 140,
            },
        ],
            ...
            {selectedCuentas.length > 0 ? (
                <Box mt={1} mb={2}>
                    <Typography variant="subtitle2">
                        Cliente:{" "}
                        <strong>{form.cliente_nombre || "Sin nombre"}</strong>
                    </Typography>
                    ...
                    <Typography variant="caption" color="text.secondary">
                        * El total del recibo se calcula como la suma de “Monto
                        a pagar”.
                    </Typography>
                </Box>
                ...
                <Box mt={1} mb={2}>
                    ...
                    <Typography variant="subtitle2">
                        ...
                        <strong>
                            Q
                            {parseFloat(form.saldo_pendiente).toLocaleString(
                                "es-GT",
                                {
                                    minimumFractionDigits: 2,
                                }
                ...
                <DialogContent>
                    ...
                    <MaterialReactTable
                        ...
                        onEditingCellSave={({ row, value, table }) => {
                            const max =
                                Number(row.original.saldo_pendiente) || 0;
                            const val = Math.min(
                            ...
                            setSelectedCuentas((prev) => {
                                ...
                                const idx = copy.findIndex(
                                    (x) =>
                                        x.idcuentaporcobrar ===
                                        row.original.idcuentaporcobrar
                                );
                                if (idx >= 0) {
                                    copy[idx] = {
                                        ...copy[idx],
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <Button
                                        ...
                                        onClick={() => {
                                            const base = selectedRows.map(
                                                (r) => r.original
                                            );
                                            const enriquecidas = base.map(
                                                (b) => {
                                                    const ya =
                                                        selectedCuentas.find(
                                                            (x) =>
