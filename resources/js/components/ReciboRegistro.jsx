...
const ReciboRegistro = () => {
      ...
      axios
        ...
        .then((res) => {
          ...
          setForm({
            ...
            saldo_pendiente: parseFloat(data.cuenta?.saldo_pendiente) || 0,
  ...
  const seleccionarCuenta = (cuenta) => {
    setClientes((prevClientes) => {
      const clienteObj = prevClientes.find(
        (c) => String(c.idcliente).trim() === String(cuenta.idcliente).trim()
      );
      ...
      setForm((prevForm) => ({
        ...
        cliente_nombre: clienteObj ? clienteObj.nombre : "No encontrado",
  ...
  const handleSubmit = (e) => {
    ...
    if (parseFloat(form.monto_recibido) > parseFloat(form.saldo_pendiente)) {
      alertify.error("El monto recibido no puede ser mayor al saldo pendiente.");
      return;
    }
    ...
    axios({
      ...
      .then(() => {
        alertify.success(id ? "Recibo actualizado correctamente" : "Recibo registrado correctamente");
  ...
  const columnas = useMemo(
    () => [
      ...
      { accessorKey: "saldo_pendiente", header: "Saldo Pendiente" },
      {
        ...
        header: "Seleccionar",
        Cell: ({ row }) => (
          <Button variant="outlined" size="small" onClick={() => seleccionarCuenta(row.original)}>
            Seleccionar
          </Button>
        ),
      },
    ...
    <Box className="container mt-4">
      ...
      <Dialog open={modalOpen} onClose={() => setModalOpen(false)} fullWidth maxWidth="md">
        ...
        <DialogContent>
          ...
          <Button variant="outlined" onClick={handleBuscarCuentas} disabled={!clienteSeleccionado} sx={{ mb: 2 }}>
            Buscar Cuentas
          </Button>
