...
const CotizacionesConsultaContabilidad = () => {
  ...
  useEffect(() => {
    axios.get(`${import.meta.env.VITE_API_URL}/fecha-servidor`, { headers })
      .then(res => {
        setDesde(res.data.fecha);
        setHasta(res.data.fecha);
      });

    axios.get(`${import.meta.env.VITE_API_URL}/reportes-contabilidad/vendedores`, { headers })
      .then(res => setVendedores(res.data));
  }, []);

  const fetchData = (page = 1) => {
    axios.get(`${import.meta.env.VITE_API_URL}/reportes-contabilidad/cotizaciones`, {
      headers,
      params: { desde, hasta, vendedor_id: vendedorId, estado, search, page, per_page: data.per_page }
    })
  ...
  const exportExcel = () => {
    axios.get(`${import.meta.env.VITE_API_URL}/reportes-contabilidad/export/excel`, {
      headers, params: { desde, hasta, vendedor_id: vendedorId, estado, search }, responseType: 'blob'
    }).then(res => {
      const url = window.URL.createObjectURL(new Blob([res.data]));
      const link = document.createElement('a');
      link.href = url; link.setAttribute('download','cotizaciones.xlsx');
    ...
    <Box sx={{ p: 2 }}>
      <Box sx={{ display:'flex', gap:2, mb:2, flexWrap:'wrap' }}>
        ...
        <FormControl sx={{minWidth:200}}>
          ...
          <Select value={vendedorId} onChange={e=>setVendedorId(e.target.value)} label="Vendedor">
            <MenuItem value="">Todos</MenuItem>
            {vendedores.map(v=>(
              <MenuItem key={v.id_empleado} value={v.id_empleado}>{v.nombre}</MenuItem>
            ))}
          </Select>
        </FormControl>
        <FormControl sx={{minWidth:200}}>
        ...
        <TextField label="Buscar" value={search}
          onChange={e=>setSearch(e.target.value)} placeholder="n° cotización o cliente"/>
        ...
        <Table>
          <TableHead>
            <TableRow>
              ...
              <TableCell>Fecha</TableCell><TableCell>Vendedor</TableCell>
              <TableCell>Cliente</TableCell><TableCell>Estado</TableCell>
          ...
          <TableBody>
            {data.data.map(row=>(
              <TableRow key={row.idcotizacion}>
                ...
                <TableCell>{new Date(row.fecha_cotizacion).toLocaleDateString()}</TableCell>
                <TableCell>{row.vendedor}</TableCell>
                <TableCell>{row.cliente}</TableCell>
                <TableCell>{estados.find(e=>e.value===row.estado)?.label}</TableCell>
                <TableCell align="right">
                  {Number(row.total_general).toLocaleString('es-GT',{style:'currency',currency:'GTQ'})}
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
