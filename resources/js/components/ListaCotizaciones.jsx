...
function ListaCotizaciones() {
  ...
  useEffect(() => {
    if (openComentarios && registroSeleccionado?.idcotizacion) {
      fetchComentarios(registroSeleccionado.idcotizacion, 1, comentariosSearch);
    }
  }, [comentariosSearch]);
  ...
  const obtenerDetalleCotizacion = async (id) => {
    ...
    try {
      const response = await axios.get(`/api/cotizaciones/detalle/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const chipEstado = (row) => {
    ...
    return (
      <Chip size="small" label={row?.estado_texto ?? ""} color={color} variant="outlined" />
    );
  };
  ...
  useEffect(() => {
    const totalPages = Math.ceil(cotizacionesFiltradas.length / rowsPerPage) || 1;
      ...
      axios
        ...
        .then(() => {
          setCotizaciones((prev) =>
            prev.filter((c) => Number(c.idcotizacion) !== Number(id))
          );
        })
        .catch(() => {
  ...
  const handleFacturar = (id, cotizacion, estado) => {
    ...
    if (!cotizacion || !cid) {
      alertify.alert("Error", "No se encontró la cotización seleccionada.");
      return;
    }
    ...
    if (token) {
      axios
        ...
        .then((response) => {
          ...
          fetchCotizaciones(fechaInicioRef.current, fechaFinRef.current, estadoFiltro);
        })
        .catch(() => {
  ...
  const generarPDF = async () => {
    ...
    if (!res.ok) return alertify.error(data.message || "Error generando PDF.");
    setPdfData(data);
  };
  ...
  const confirmarRechazo = async () => {
    ...
    try {
      ...
      alertify.error(error.response?.data?.message || "Error al rechazar.");
    }
  };
  ...
  const obtenerHistorialEnvios = async (id) => {
    ...
    if (!token) return alertify.error("Token no encontrado");
    try {
      const res = await fetch(`/api/cotizaciones/${id}/historial-envios`, {
        headers: { Authorization: `Bearer ${token}` },
      });
    ...
    <div className="mt-4 px-3 px-md-4">
      ...
      {pdfData && (
        ...
        >
          ...
          <div className="mt-3 d-flex gap-2">
            ...
            <button className="btn btn-danger" onClick={() => setPdfData(null)}>
              Cerrar PDF
            </button>
          </div>
        </div>
        ...
        <div className="card-body">
          ...
          <div className="mb-3">
            <div className="row g-3 align-items-center">
              ...
              <div className="col-auto">
                <label htmlFor="estadoFiltro" className="form-label">
                  Estado:
                </label>
              </div>
              <div className="col-md-3">
          ...
          <div className="mb-3">
            ...
            <div className="input-group">
              ...
              />
              {filtro && (
                <button className="btn btn-outline-secondary" onClick={limpiarFiltro}>
                  ✖
                </button>
              )}
            </div>
          ...
          <Stack direction="row" spacing={1.5} className="mb-3" alignItems="center" flexWrap="wrap">
            <MUIButton
              variant="contained"
              size="small"
          ...
          <Menu anchorEl={actionsAnchor} open={Boolean(actionsAnchor)} onClose={closeActions}>
            ...
            <MenuItem
              onClick={() => {
                closeActions();
                obtenerDetalleCotizacion(registroSeleccionado?.idcotizacion);
              }}
              disabled={!registroSeleccionado}
            >
              <ListItemIcon>
            ...
            <MenuItem
              onClick={() => {
                ...
                fetchComentarios(registroSeleccionado.idcotizacion, 1, "");
              }}
              disabled={!registroSeleccionado}
            >
              <ListItemIcon>
            ...
            <MenuItem
              ...
              disabled={!puedeEnviarCosteo || !registroSeleccionado}
            >
              <ListItemIcon>
                <Send fontSize="small" />
              </ListItemIcon>
            ...
            <MenuItem
              ...
              disabled={!puedePreFacturar || !registroSeleccionado}
            >
              <ListItemIcon>
            ...
            <MenuItem
              ...
              disabled={!puedeEnviarAFacturacion || !registroSeleccionado}
            >
              <ListItemIcon>
                <Send fontSize="small" />
              </ListItemIcon>
            ...
            <MenuItem
              onClick={() => {
                closeActions();
                navigate(`/cotizaciones/editar/${registroSeleccionado?.idcotizacion}`);
              }}
            ...
            <MenuItem
              ...
              disabled={!registroSeleccionado || !(estado === 1 || estado === 3)}
            >
              <ListItemIcon>
          ...
          <Paper elevation={1} sx={{ width: "100%", overflow: "hidden", borderRadius: 2 }}>
            <TableContainer sx={{ maxHeight: 560 }}>
              <Table stickyHeader size="small" aria-label="cotizaciones">
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell>Fecha Prefacturación</TableCell>
                    ...
                    rowsToShow.map((row) => {
                      ...
                      return (
                        <TableRow
                          ...
                          onClick={() => {
                            setSelectedId(row.idcotizacion);
                        ...
                        >
                          ...
                          <TableCell>{fmtFecha(row.fecha_cotizacion)}</TableCell>
                          <TableCell>{row.tipo_pago}</TableCell>
                          <TableCell align="right">{fmtMoney(row.total_general)}</TableCell>
                          <TableCell>{row.costear}</TableCell>
                          <TableCell>{row.cliente}</TableCell>
                          <TableCell>{row.contacto}</TableCell>
                          <TableCell>{row.observaciones_costeo}</TableCell>
                          <TableCell>{row.costeo_observaciones}</TableCell>

                          <TableCell align="center">
                            {(() => {
                              const isPre = Number(row?.estado) === 4;
                              const hasCom =
                                Number(row?.comentarios_count ?? row?.has_comentarios ?? 0) > 0 ||
                                Number(row?.has_comentarios) === 1 ||
                                row?.has_comentarios === true;

                              const disabled = !(isPre && hasCom);
                              const badgeColor = !disabled ? "warning" : "default";
                                ...
                                <Tooltip
                                  title={
                                    <>
                                      ...
                                      {row.last_comentario_at && hasCom && (
                                        <small>
                                          {new Date(row.last_comentario_at).toLocaleString()}
                                        </small>
                                      )}
                                    </>
                                  }
                                  ...
                                  <span>
                                    <IconButton
                                      ...
                                      onClick={(e) => {
                                        ...
                                        setSelectedId(row.idcotizacion);
                                        setRegistroSeleccionado(row);
                                        fetchComentarios(row.idcotizacion, 1, "");
                                      }}
                                    >
                                      <Badge
                                        badgeContent={Number(row?.comentarios_count) || 0}
                                        color={badgeColor}
                                      >
                                        <ChatBubbleIcon />
                                      </Badge>
                          ...
                          <TableCell>{chipEstado(row)}</TableCell>
                          <TableCell>{fmtFecha(row.fecha_prefacturacion)}</TableCell>
          ...
          <div className="modal-dialog">
            <div className="modal-content">
              <div className="modal-header">
                ...
                <button className="btn-close" onClick={() => setMostrarModalRechazo(false)}></button>
              </div>
              <div className="modal-body">
                ...
                >
                  ...
                  {motivosRechazo.map((m) => (
                    <option key={m.idmotivorechazo} value={m.idmotivorechazo}>
                      {m.motivo}
                    </option>
                  ))}
                </select>
              ...
              <div className="modal-footer">
                <button className="btn btn-secondary" onClick={() => setMostrarModalRechazo(false)}>
                  Cancelar
                </button>
                <button className="btn btn-danger" onClick={confirmarRechazo}>
                  Rechazar
                </button>
              </div>
            </div>
        ...
        <div className="modal d-block" style={{ backgroundColor: "rgba(0,0,0,0.5)" }}>
          <div className="modal-dialog modal-lg">
            <div className="modal-content">
              <div className="modal-header">
                <h5>Generar PDF</h5>
                <button className="btn-close" onClick={() => setShowPdfModal(false)}></button>
              </div>
              <div className="modal-body">
                  ...
                  <div className="mt-3">
                    ...
                    <ul className="list-group">
                      {historialEnvios.map((item) => (
                        ...
                        >
                          <span>📄 Fecha cotización: {item.fecha_cotizacion}</span>
                          <span>🕒 Enviado: {new Date(item.fecha_envio).toISOString().slice(0, 10)}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
              ...
              <div className="modal-footer">
                <button
                  ...
                  onClick={async () => {
                    try {
                      if (!registroSeleccionado?.idcotizacion) {
                        alertify.error("Cotización no seleccionada.");
                        return;
                      }
                      await generarPDF();
                    } catch (err) {
                      alertify.error(err?.message || "Error generando PDF");
                    } finally {
      ...
      {showNotaEnvioModal && registroSeleccionado && (
        <NotaEnvioModal
          ...
          direccionSugerida={registroSeleccionado.direccion_entrega || ""}
        />
      )}
        ...
        >
          ...
          <div className="mt-3 d-flex gap-2">
            <PDFDownloadLink
              ...
              className="btn btn-primary"
            >
              {({ loading }) => (loading ? "Generando PDF..." : "Descargar Nota de Envío")}
            </PDFDownloadLink>

            <button className="btn btn-danger" onClick={() => setNotaEnvioPayload(null)}>
              Cerrar PDF
            </button>
          </div>
        </div>
            ...
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">
                  Comentarios — Cotización {registroSeleccionado?.nocotizacion}
                </h5>
                <button className="btn-close" onClick={() => setOpenComentarios(false)}></button>
              </div>
              <div className="modal-body">
                <div className="mb-2">
                  <input
                    ...
                    onChange={(e) => setComentariosSearch(e.target.value)}
                  />
                </div>
                  ...
                  <>
                    {comentariosPag.data.map((c, i) => (
                      <div key={i} className="border rounded p-2 mb-2">
                        ...
                        <div className="text-muted" style={{ fontSize: "0.85rem" }}>
                          Usuario: {c.nombre_usuario ?? c.idusuario} ·{" "}
                          {new Date(c.fecha_registro).toLocaleString()} · Estado: {c.estado}
                        </div>
                      </div>
                    ))}
                    ...
                    <div className="d-flex justify-content-between mt-3">
                      <button
                        ...
                        disabled={!comentariosPag?.prev_page_url}
                        onClick={() =>
                          fetchComentarios(
                            registroSeleccionado.idcotizacion,
                            comentariosPage - 1,
                      ...
                      <button
                        className="btn btn-outline-secondary btn-sm"
                        disabled={!comentariosPag?.next_page_url}
                        onClick={() =>
                          fetchComentarios(
                            registroSeleccionado.idcotizacion,
                            comentariosPage + 1,
              ...
              <div className="modal-footer">
                <button className="btn btn-secondary" onClick={() => setOpenComentarios(false)}>
                  Cerrar
                </button>
              </div>
            </div>
