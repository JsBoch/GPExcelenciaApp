...
<table class="detalle">
    ...
    <tbody>
        ...
        @foreach ($cuentas as $c)
            @php
                $totalOriginal += $c->monto_original;
                $totalPagado += $c->monto_pagado;
                $totalPendiente += $c->saldo_pendiente;
            @endphp
            <tr>
                ...
                <td>{{ $c->moneda }}</td>
                <td>Q {{ number_format($c->monto_original, 2) }}</td>
                <td>Q {{ number_format($c->monto_pagado, 2) }}</td>
                <td>Q {{ number_format($c->saldo_pendiente, 2) }}</td>
            </tr>
        @endforeach
    </tbody>
</table>
